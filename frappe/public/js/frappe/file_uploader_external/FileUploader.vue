	...
	methods: {
		...
		upload_file(file, i) {
			...
			return new Promise((resolve, reject) => {
				...
				// set baseurl for upload
				xhr.open('POST', 'https://pinisi-integrator.teamdev.id/dms/upload', true);
				// set Access-Control-Allow-Origin header * for upload
				xhr.setRequestHeader('Access-Control-Allow-Origin', '*');
				// set Access-Control-Allow-Headers header * for upload
				xhr.setRequestHeader('Access-Control-Allow-Headers', '*');
				// set Access-Control-Allow-Methods header GET,PUT,POST,DELETE,PATCH,OPTIONS for upload
				xhr.setRequestHeader('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,PATCH,OPTIONS');
				// set Authorization Basic token for upload
				xhr.setRequestHeader('Authorization', 'Basic ' + 'bWl0cmF0ZWxfcGluaXNpOk1pdHJhdGVsMTIzIQ==');

				xhr.setRequestHeader('Accept', 'application/json');
				// xhr.setRequestHeader('X-Frappe-CSRF-Token', frappe.csrf_token);

				var docName = this.docname.replaceAll(/ /g, "_");
				var docTypeName = this.doctype.replaceAll(/ /g, "_");
				// generate file name dd-mm-yyyy hh:mm:ss
				var date = new Date();
				...
				var folderFileName = dd + '-' + mm + '-' + yyyy + '-' + hh + ':' + min + ':' + ss;
				let form_data = new FormData();
				if (file.file_obj) {
					form_data.append('document', file.file_obj, file.name);
				}
				form_data.append('name', folderFileName +'/'+file.name);
				// form_data.append('is_private', +file.private);
				form_data.append('group', docTypeName);
